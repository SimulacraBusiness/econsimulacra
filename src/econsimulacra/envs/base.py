...
class Environment(Generic[ObsT]):
    ...
    def generate_invalid_action_log(
        self,
        agent_id: int,
        action_type: str,
        description: str,
    ) -> None:
        log: InvalidActionLog = InvalidActionLog(
            time=self.get_time(),
            time_step=self.get_time_step(),
            agent_id=agent_id,
    ...
    def _check_sleep_duration(
        ...
        if sleep_manager is None:
            self.generate_invalid_action_log(
                agent_id=agent_id,
                action_type="sleep",
                description="SleepManager is not available.",
            )
            return False
        is_sleeping: bool = sleep_manager.get_sleep_status(agent_id=agent_id)
        if is_sleeping:
            self.generate_invalid_action_log(
                agent_id=agent_id,
                action_type="sleep",
                description="You are already sleeping.",
            )
            return False
        ...
        if current_pos != initial_coords:
            self.generate_invalid_action_log(
                agent_id=agent_id,
                action_type="sleep",
                description="You are not in your house. Go back your home before trying to sleep.",
            )
            return False
        if isinstance(sleep_duration, str):
            ...
            if time_unit not in ["m", "h"]:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="sleep",
                    description=f"Invalid format in sleep_duration '{sleep_duration}'. Use, for example, '30m' for 30minutes or '8h' for 8 hours.",
                )
                return False
            try:
                ...
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="sleep",
                    description=f"Invalid time value '{time_value_str}' in sleep_duration '{sleep_duration}'.",
                )
                return False
            if time_value <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="sleep",
                    description=f"Invalid time value '{time_value}' in sleep_duration '{sleep_duration}'. Must be a positive number.",
                )
                return False
            ...
            if sleep_duration <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="sleep",
                    description=f"Invalid sleep duration '{sleep_duration}'. Must be a positive number.",
                )
                return False
        else:
            return False
    ...
    def _check_move(
        ...
        if len(destination_pos) != len(space_size):
            self.generate_invalid_action_log(
                agent_id=agent_id,
                action_type="move",
                description=f"Invalid destination {destination_pos}. The dimension of the destination does not match the dimension of the environment space {space_size}.",
            )
            return False
        for dim in range(len(destination_pos)):
            if not (0 <= destination_pos[dim] < space_size[dim]):
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="move",
                    description=f"Invalid destination {destination_pos}. The position is out of bounds.",
                )
                return False
        return True
        ...
        for consumption in consumptions:
            ...
            if item_name not in self.item_name2item:
                self.generate_invalid_action_log(
                    ...
                    description=f"You requested to consume '{item_name}', which does not exist in the environment.",
                )
                continue
            ...
            if item_amount > allowed_amount:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="consumption",
                    description=f"You requested to consume {item_amount} of '{item_name}', which exceeds your available amount of {allowed_amount}.",
                )
                continue
            if item_amount <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="consumption",
                    description=f"You specified an invalid consumption amount of {item_amount} for '{item_name}'. The amount must be a positive number.",
                )
                continue
        ...
        for order_dic in orders:
            ...
            if counterparty_id is None and counterparty_name is None:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description="You did not specify either counterparty_id or counterparty_name for your order.",
                )
                continue
            ...
            if counterparty_id not in self.agent_ids:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You specified counterparty_id {counterparty_id}, which does not exist in the environment.",
                )
                continue
            if "item_name" not in order_dic:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description="You did not specify item_name for your order.",
                )
                continue
            ...
            if item_name not in self.item_name2item:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You specified item_name {item_name}, which does not exist in the environment.",
                )
                continue
            if item_name == self.cash_name:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You placed an order for cash ({self.cash_name}), which is not allowed.",
                )
                continue
            if agent_id not in self.grid_space.get_colocated_agents(
                ...
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You were not in the same cell as the counterparty agent {counterparty_id}. You must be in the same cell to place an order.",
                )
                continue
            ...
            if item_amount <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You specified an invalid item amount ({item_amount}) for item {item_name}. The amount must be positive.",
                )
                continue
            ...
            if total_cost > cash_amount:
                ...
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="order",
                    description=f"You placed orders totaling {total_cost} cash, which exceeded your available cash of {cash_amount}.",
                )
                continue
            else:
        ...
        for proposal_dic in proposals:
            ...
            if responder_agent_id is None and responder_agent_name is None:
                self.generate_invalid_action_log(
                    ...
                    description="You did not specify either responder_agent_id or responder_agent_name for your proposal.",
                )
                continue
            ...
            if responder_agent_id not in self.agent_ids:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description=f"You specified responder_agent_id {responder_agent_id}, which does not exist in the environment.",
                )
                continue
            if (
                ...
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description="You did not specify all required fields: give_item_name, give_item_amount, get_item_name, get_item_amount for your proposal.",
                )
                continue
            ...
            if give_item_name not in self.item_name2item:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description=f"You specified an invalid give_item_name '{give_item_name}', which does not exist in the environment.",
                )
                continue
            if get_item_name not in self.item_name2item:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description=f"You specified an invalid get_item_name '{get_item_name}', which does not exist in the environment.",
                )
                continue
            if give_item_amount <= 0 or get_item_amount <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description="You specified a non-positive item amount for your proposal.",
                )
                continue
            ...
            if give_item_amount > allowed_give_amount:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="proposal",
                    description=f"You specified a give_item_amount '{give_item_amount}' for your proposal, which exceeds your available inventory of '{give_item_name}'.",
                )
                continue
        ...
        if follow_agent_id is not None:
            if follow_agent_id not in self.agent_ids:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="follow",
                    description=f"You specified follow_agent_id {follow_agent_id}, which does not exist in the environment.",
                )
                valid_follow_agent_id = None
            if follow_agent_id in self.social_network.get_follows(agent_id=agent_id):
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="follow",
                    description=f"You specified follow_agent_id {follow_agent_id}, which you were already following.",
                )
                valid_follow_agent_id = None
        ...
        if unfollow_agent_id is not None:
            if unfollow_agent_id not in self.agent_ids:
                self.generate_invalid_action_log(
                    ...
                    description=f"You specified unfollow_agent_id {unfollow_agent_id}, which does not exist in the environment.",
                )
                valid_unfollow_agent_id = None
            if unfollow_agent_id not in self.social_network.get_follows(
                ...
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="unfollow",
                    description=f"You specified unfollow_agent_id {unfollow_agent_id}, which you were not following.",
                )
                valid_unfollow_agent_id = None
        ...
        if allowed_num_follows is not None:
            ...
            if allowed_num_follows < 0:
                self.generate_invalid_action_log(
                    ...
                    description=f"Performing the follow and unfollow actions would result in exceeding the follow cap of {self.social_network.follow_cap}.",
                )
                valid_follow_agent_id = None
        ...
        for reaction in reactions:
            if "kind" not in reaction:
                self.generate_invalid_action_log(
                    ...
                    description="You did not specify the 'kind' of your reaction. Each reaction must include a 'kind' key with value either 'order' or 'proposal'.",
                )
                continue
            ...
            if kind == "order":
                if "id" not in reaction or "accept_amount" not in reaction:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description="You did not include 'id' or 'accept_amount' keys in your order reaction.",
                    )
                    continue
                ...
                if order_id in reacted_order_ids:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description=f"You reacted to order {order_id} more than once in the same step, which is not allowed.",
                    )
                    continue
                ...
                if accept_amount < 0:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description=f"You specified a negative 'accept_amount' for order {order_id}.",
                    )
                    continue
                    ...
                    if order.order_id == order_id and order.counterparty_id == agent_id:
                        if accept_amount > order.item_amount:
                            self.generate_invalid_action_log(
                                agent_id=agent_id,
                                action_type="reaction",
                                description=f"You specified 'accept_amount' {accept_amount} for order {order_id}, which exceeded the order's item amount of {order.item_amount}.",
                            )
                            continue
                        ...
                        if accept_amount > holding_amount:
                            self.generate_invalid_action_log(
                                agent_id=agent_id,
                                action_type="reaction",
                                description=f"You specified 'accept_amount' {accept_amount} for order {order_id}, which exceeded your holding amount of {holding_amount}.",
                            )
                            continue
                ...
                if not find_corresponding_order:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description=f"You did not specify a valid order id in your reaction. Order id {order_id} did not correspond to any existing pending order where you are the counterparty.",
                    )
                    continue
                ...
                if "id" not in reaction or "accept" not in reaction:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description="You did not include 'id' or 'accept' keys in your proposal reaction.",
                    )
                    continue
                ...
                if proposal_id in reacted_proposal_ids:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description=f"You reacted to proposal {proposal_id} more than once in the same step, which is not allowed.",
                    )
                    continue
                        ...
                        if accept:
                            ...
                            if proposal.get_item_amount > holding_amount:
                                self.generate_invalid_action_log(
                                    agent_id=agent_id,
                                    action_type="reaction",
                                    description=f"You specified 'accept_amount' {proposal.get_item_amount} for proposal {proposal_id}, which exceeded your holding amount of {holding_amount}.",
                                )
                                continue
                ...
                if not find_corresponding_proposal:
                    self.generate_invalid_action_log(
                        agent_id=agent_id,
                        action_type="reaction",
                        description=f"You did not specify a valid proposal id in your reaction. Proposal id {proposal_id} did not correspond to any existing pending proposal where you are the counterparty.",
                    )
                    continue
                valid_reactions.append(reaction)
            else:
        ...
        if agent_id in self.household_ids:
            if 0 < len(set_prices):
                self.generate_invalid_action_log(
                    ...
                    description="You tried to set prices, but you are a household and not allowed to set prices.",
                )
            return []
        agent: Agent = self.agent_id2agent[agent_id]
        for set_price in set_prices:
            if "item_name" not in set_price or "price" not in set_price:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="set_price",
                    description="You did not specify 'item_name' or 'price' in your set_price action.",
                )
                continue
            ...
            if item_name not in self.item_name2item:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="set_price",
                    description=f"You tried to set the price for item '{item_name}', but it does not exist in the environment.",
                )
                continue
            if agent.get_item_amount(item_name) <= 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="set_price",
                    description=f"You tried to set the price for item '{item_name}', but you do not have any of it.",
                )
                continue
            if price < 0:
                self.generate_invalid_action_log(
                    agent_id=agent_id,
                    action_type="set_price",
                    description=f"You tried to set the price for item '{item_name}', but the price is negative.",
                )
                continue
