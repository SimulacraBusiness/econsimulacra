...
from typing import Any, Optional, Protocol, TypeAlias, cast
...
class ConsumerClusterAnalyzer(AnalyzerBase[ConsumerClusterResult]):
    ...
    cluster_centers_: Optional[ConsumerVector] = None
    labels_: Optional[ClusterLabelArray] = None
    vectors_: Optional[ConsumerVector] = None
    agent_ids_: Optional[list[int]] = None
    window_starts_: Optional[list[TimeKey]] = None
    stability_scores_: Optional[dict[int, float]] = None
    ...
    def analyze(self, store: RecordStore) -> ConsumerClusterResult:
        ...
        self.cluster_centers_ = x_matrix[closest_indices]
    ...
    def draw_figs(self, result: ConsumerClusterResult) -> dict[str, Figure]:
        ...
        for (src_label, dst_label), count in transition_counts.items():
            ...
            line_width: float = 1.0 + 2.0 * float(count) / float(max_count)
        ...
        times: list[TimeKey] = sorted(time2cluster_counts.keys())
        cluster_ids: list[int] = sorted(
            {
                cluster_id
                for cluster_counts in time2cluster_counts.values()
        ...
        for cluster_id in cluster_ids:
            counts = np.array(
                [time2cluster_counts[time][cluster_id] for time in times],
                dtype=float,
            )
            ax_cluster_counts.bar(
                x,
                counts,
                bottom=bottom,
            ...
            bottom += counts
        ax_cluster_counts.set_xlabel("Time")
        ax_cluster_counts.set_ylabel("Number of agents")
